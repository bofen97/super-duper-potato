...
class Actor(object):
    ...
    def sample(self):
        ...
        for i in range(self.config['sample_batch_steps']):
            ...
            for env_id in range(self.config['env_num']):
                ...
                if i == self.config['sample_batch_steps'] -1:
                    ...
                    sample_data['value'].extend(env_values.squeeze(1))
                    sample_data['rew'].extend(env_rewards)
        ...
        train_data = {
            ...
            'value':np.array(sample_data['value'],dtype='float32'),
            'rew':np.array(sample_data['rew'],dtype='float32')
